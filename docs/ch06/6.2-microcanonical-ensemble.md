...
??? question "Check Your Understanding"
    1. You can define entropy using $\Sigma$, $\Gamma$, or $\omega$. They differ by terms of order $\log N$. For $N = 10^{23}$, that's a correction of about 53. Sounds big. Why doesn't it matter?
    2. You split your 108-atom box in half and measure the energy of each half over time. Is the energy split rock-solid or sloshing around? What about for $10^{23}$ atoms?
    3. We showed that at equilibrium, $T_1 = T_2$ for two subsystems in thermal contact. Could you actually use this in practice? Like, split your simulation box in half, measure $T$ for each half, and check if your run has equilibrated?
    4. The second law says entropy only increases. But stat mech only compares equilibrium states. So what happens *during* an irreversible process? Can you describe it with the tools we have so far?
